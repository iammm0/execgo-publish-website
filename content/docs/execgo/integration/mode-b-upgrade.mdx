---
title: "模式 B 升级路径（translate + TaskGraph）"
description: "模式 B 面向需要 直接操作 TaskGraph 或先 只翻译、不执行 再上线的团队，与模式 A 可并存。"
---
...
须符合 [Task DSL](/docs/execgo/reference/task-dsl) 校验。
...
- `adapter.v1` 见 [execgocli 契约](/docs/execgo/reference/execgo-cli-contract)。
...
- [模式 A 快速开始](/docs/execgo/integration/mode-a-cli)
