---
title: "通用 Agent 可靠执行层接入"
description: "ExecGo 现在有两条 HTTP 接入路径："
---
...
**通用 CLI：** 在仓库内构建 `execgocli`（`cmd/execgocli`）即可用稳定 JSON 外壳走同一套 HTTP 流程，见 [模式 A 快速开始（CLI）](/docs/execgo/integration/mode-a-cli) 与 [execgocli JSON 契约](/docs/execgo/reference/execgo-cli-contract)。**模式 B**（`translate` + `POST /tasks`）见 [模式 B 升级路径](/docs/execgo/integration/mode-b-upgrade)。
