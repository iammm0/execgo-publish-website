---
title: "架构说明"
description: "execgo-runtime 是 ExecGo 的执行后端（数据面）：接收任务描述、落盘、调度执行，并通过 HTTP 暴露状态与运维接口。控制面（如 ExecGo 自身）通过 HTTP 与本服务交互，不直接 fork 用户进程。"
---
...
详见 [api.md](/docs/runtime/api) 中的沙箱字段说明。
