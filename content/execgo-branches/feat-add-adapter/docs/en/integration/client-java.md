# Java (HTTP) Integration Example

This example uses Java `HttpClient` to integrate with ExecGo over HTTP:

- `POST /tasks` submit a `TaskGraph`
- read `task_ids`
- poll `GET /tasks/{id}` and read `status/result/error`

The example uses the built-in `noop` executor (no external I/O) so you can run it locally end-to-end.

## Prerequisite：Jackson for JSON parsing (recommended)

Add Jackson to your `pom.xml` (choose a version that matches your project):
...
Make sure ExecGo is running at `localhost:8080` (or set `EXECGO_URL`).
